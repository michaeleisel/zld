...
void IMPCachesBuilder::buildTrackedClasses(SharedCacheBuilder::DylibInfo& dylib, const dyld3::MachOAnalyzer* ma) {
    ...
    ma->forEachObjCClass(_diagnostics, vmAddrConverter, visitClass);
}
...
void IMPCachesBuilder::populateMethodLists(SharedCacheBuilder::DylibInfo& dylib, const dyld3::MachOAnalyzer* ma, int* duplicateClassCount) {
...
void IMPCachesBuilder::attachCategories(SharedCacheBuilder::DylibInfo& dylib, const dyld3::MachOAnalyzer* ma) {
    dyld3::MachOAnalyzer::VMAddrConverter vmAddrConverter = ma->makeVMAddrConverter(false);
    ...
    ma->forEachObjCCategory(_diagnostics, vmAddrConverter, visitCategory);
}
...
void IMPCachesBuilder::inlineSelectors(SharedCacheBuilder::DylibInfo& dylib, std::unordered_map<std::string_view, SharedCacheBuilder::DylibInfo*> & dylibsByInstallName, const dyld3::MachOAnalyzer* ma) {
    dyld3::MachOAnalyzer::VMAddrConverter vmAddrConverter = ma->makeVMAddrConverter(false);
    auto visitClass = ^(uint64_t classVMAddr, uint64_t classSuperclassVMAddr, uint64_t classDataVMAddr, const dyld3::MachOAnalyzer::ObjCClassInfo& objcClass, bool isMetaClass, bool& stop) {
        ...
        while (!isRootClass) {
            ...
            SharedCacheBuilder::DylibInfo* classDylib = dylibsByInstallName.at(currentMA->installName());
...
bool IMPCachesBuilder::parseDylibs(Diagnostics& diag) {
    std::unordered_map<std::string_view, SharedCacheBuilder::DylibInfo*> dylibsByInstallName;
    
    for (SharedCacheBuilder::DylibInfo& d : dylibs) {
...
void IMPCachesBuilder::buildClassesMap(Diagnostics& diag) {
    ...
    for (SharedCacheBuilder::DylibInfo& dylib : dylibs) {
...
IMPCachesBuilder::IMPCachesBuilder(std::vector<SharedCacheBuilder::DylibInfo>& allDylibs, const dyld3::json::Node& optimizerConfiguration, Diagnostics& diag, TimeRecorder& timeRecorder, const dyld3::closure::FileSystem& fileSystem) : dylibs(allDylibs), _diagnostics(diag), _timeRecorder(timeRecorder), _fileSystem(fileSystem) {
...
void IMPCachesBuilder::fillAllClasses(std::vector<IMPCaches::ClassData*> & allClasses) {
    for (const SharedCacheBuilder::DylibInfo & d : dylibs) {
