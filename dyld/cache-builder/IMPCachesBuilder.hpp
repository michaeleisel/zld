...
public:
    ...
    std::vector<SharedCacheBuilder::DylibInfo> & dylibs;

    IMPCachesBuilder(std::vector<SharedCacheBuilder::DylibInfo>& allDylibs, const dyld3::json::Node& optimizerConfiguration, Diagnostics& diag, TimeRecorder& timeRecorder, const dyld3::closure::FileSystem& fileSystem);
...
private:
    ...
    void buildTrackedClasses(SharedCacheBuilder::DylibInfo& dylib, const dyld3::MachOAnalyzer* ma);

    /// Parses the method lists in the source dylibs to determine what will end up in which IMP cache.
    void populateMethodLists(SharedCacheBuilder::DylibInfo& dylib, const dyld3::MachOAnalyzer* ma, int* duplicateClassCount);

    /// Go through categories and add the methods from the category to the corresponding class's IMP cache
    void attachCategories(SharedCacheBuilder::DylibInfo& dylib, const dyld3::MachOAnalyzer* ma);

    // Inline some selectors (driven by the OrderFiles) from parent caches into child caches.
    void inlineSelectors(SharedCacheBuilder::DylibInfo& dylib, std::unordered_map<std::string_view, SharedCacheBuilder::DylibInfo*> & dylibsByInstallName, const dyld3::MachOAnalyzer* ma);
