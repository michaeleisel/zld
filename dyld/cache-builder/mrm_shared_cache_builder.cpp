...
struct BuildInstance {
    ...
    std::vector<SharedCacheBuilder::CacheBuffer>    cacheData;
    SharedCacheBuilder::CacheBuffer                 symbolFileData;
...
static void validiateBuildOptions(const BuildOptions_v1* options, MRMSharedCacheBuilder& builder) {
...
static bool platformIsForSimulator(Platform platform) {
    return dyld3::MachOFile::isSimulatorPlatform((dyld3::Platform)platform);
}
...
bool runSharedCacheBuilder(struct MRMSharedCacheBuilder* builder) {
    ...
    builder->runSync(^() {
        ...
        auto addCacheConfiguration = ^(bool isOptimized, bool shouldEmitLocalSymbolsFile) {
            for (uint64_t i = 0; i != builder->options->numArchs; ++i) {
                ...
                if ( builder->options->platform == Platform::macOS || builder->options->platform == Platform::driverKit)
                    cacheSuffix = "";
                std::string runtimePath = IPHONE_DYLD_SHARED_CACHE_DIR;
                ...
                const char *loggingSuffix = (builder->options->platform == Platform::driverKit ? ".driverKit" : "");
                options->loggingPrefix = std::string(builder->options->deviceName) + dispositionName(builder->options->disposition) + "." + builder->options->archs[i] + cacheSuffix + loggingSuffix;
                if ( shouldEmitLocalSymbolsFile )
                    options->localSymbolsPath = runtimePath + "dyld_shared_cache_" + builder->options->archs[i] + ".symbols";
        ...
        if ( builder->options->platform == Platform::driverKit) {
            // DriverKit should always only have a development cache
            addCacheConfiguration(false, true);
        } else {
            switch (builder->options->disposition) {
                ...
                case Disposition::InternalDevelopment:
                    ...
                    if (builder->options->platform == Platform::macOS) {
                        ...
                        addCacheConfiguration(false, false);
                        addCacheConfiguration(true, true);
                    }
                    break;
                case Disposition::Customer:
                    addCacheConfiguration(true, true);
        ...
        for (auto& buildInstance : builder->builders) {
            ...
            if (cacheBuilder->errorMessage().empty()) {
                cacheBuilder->writeBuffers(buildInstance.cacheData);
                if ( !buildInstance.options->localSymbolsPath.empty() )
                    cacheBuilder->writeSymbolFileBuffer(buildInstance.symbolFileData);
        ...
        for (auto& buildInstance : builder->builders) {
            // FIXME: We can only emit the UUID for the first cache file
            ...
            cacheBuildResult.uuidString             = buildInstance.cacheData.empty() ? "" : buildInstance.cacheData.front().uuid.c_str();
            ...
            std::string cachePathSuffix = "";
            for (uint32_t cacheIndex = 0; cacheIndex != buildInstance.cacheData.size(); ++cacheIndex) {
                buildInstance.cachePaths.push_back(buildInstance.options->outputFilePath + cachePathSuffix);
                cachePathSuffix = std::string(".") + dyld3::json::decimal(cacheIndex + 1);
            }

            uint32_t cacheIndex = 0;
            for (SharedCacheBuilder::CacheBuffer& cacheBuffer : buildInstance.cacheData) {
            ...
            // Write the symbols file if we have one
            if ( buildInstance.symbolFileData.bufferSize != 0 ) {
                FileResult cacheFileResult;
                cacheFileResult.version          = 1;
                cacheFileResult.path             = buildInstance.options->localSymbolsPath.c_str();
                cacheFileResult.behavior         = AddFile;
                cacheFileResult.data             = buildInstance.symbolFileData.bufferData;
                cacheFileResult.size             = buildInstance.symbolFileData.bufferSize;
                cacheFileResult.hashArch         = buildInstance.options->archs->name();
                cacheFileResult.hashType         = buildInstance.cdHashType.c_str();
                cacheFileResult.hash             = buildInstance.symbolFileData.cdHash.c_str();
            ...
            if ( !buildInstance.macOSMap.empty() ) {
                ...
                cacheFileResult.hashArch         = buildInstance.options->archs->name();
                cacheFileResult.hashType         = buildInstance.cdHashType.c_str();
                cacheFileResult.hash             = buildInstance.cacheData.empty() ? "" : buildInstance.cacheData.front().cdHash.c_str();
