...
{
	LDOrderedMap<const char *,  generic::dylib::File*> syntheticDylibs;
	LDOrderedSet<const generic::dylib::File*> potentiallyDeadDylibs;
...
int OutputFile::compressedOrdinalForAtom(const ld::Atom* target) const
{
	...
	if ( dylib != NULL ) {
		LDOrderedMap<const ld::dylib::File*, int>::const_iterator pos = _dylibToOrdinal.find(dylib);
...
void OutputFile::writeMapFile(ld::Internal& state)
{
	if ( _options.generatedMapPath() != NULL ) {
		...
		if ( mapFile != NULL ) {
			...
			__block LDOrderedMap<std::string, const ld::File*> ltoSymbolsMap;
			...
			if ( atom->section().type() == ld::Section::typeCode ) {
				...
				for (ld::Atom::LineInfo::iterator lit = atom->beginLineInfo(); lit != atom->endLineInfo(); ++lit) {
					if ( lit->fileName != curFile ) {
                		auto ldFileName = LDString(lit->fileName);
